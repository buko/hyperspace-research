...
-->
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:dc="http://purl.org/dc/elements/1.1/">
	<!-- mozilla doesn't parse method xhtml (in xslt 2.0) -->
	<xsl:output method="xml" version="1.0" encoding="UTF-8" doctype-public="-//W3C//DTD XHTML 1.1//EN" doctype-system="http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd" omit-xml-declaration="no"/>
...
(assumed to be in ./icons directory), default is true -->
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			...
			<!-- Thanks to gulpman: -->
			<!-- added metada support on july 2007 ; Berteh -->
			...
			<head>
				...
				<xsl:element name="script">
					...
					<xsl:attribute name="src">
						<xsl:value-of select="$destination_dir"/>marktree.js</xsl:attribute>&#160; </xsl:element>
				<!-- metadata: header and metadata of the root note (duplicated in the <head> and <li> elements -->
	...
	we surround the ID with FM<ID>FM -->
			...
			<xsl:call-template name="metadata_content"/>
	...
	<xsl:template match="font">
		<xsl:if test="string-length(@SIZE) > 0">font-size:<xsl:value-of select="round((number(@SIZE) div 12)*100)"/>%;</xsl:if>
	...
	<xsl:template name="output-node-with-link">
		...
		<xsl:if test="$show_link_url='true'"> - [ <a>
				<xsl:attribute name="href">
					<xsl:value-of select="@LINK"/>
				</xsl:attribute>
				<xsl:value-of select="@LINK"/>
			...
			<a>
				...
				<xsl:choose>
					<xsl:when test="$show_icons='false'">
						<xsl:value-of select="concat('&amp;','rArr',';')" disable-output-escaping="yes"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:element name="img">
							<xsl:attribute name="src"><xsl:value-of select="$destination_dir"/>ilink.png</xsl:attribute>
	...
	<!-- metadata support -->
	<!-- currently supported: 
		* Dubin Core (http://dublincore.org/documents/usageguide/) 
		* sample tags: author, keywords-->

	<xsl:template name="metadata_header">
		<!--<xsl:if test="//node/attribute[starts-with(@NAME,'DC.')]">-->
		<link rel="schema.DC" href="http://purl.org/dc/elements/1.1/"/>
		<meta name="DC.title" content="{map/node/@TEXT}"/>
	...
	<xsl:template name="metadata_content">
		<!-- Dublin Core attributes -->
		<xsl:for-each select="attribute[starts-with(@NAME,'DC.')]">
			<meta name="{@NAME}" content="{@VALUE}"/>
		</xsl:for-each>
		<!-- proprietary attributes translated to Dublin Core: author/auteur, keywords/mots-clés -->
		<xsl:for-each select="attribute[@NAME='author' or @NAME='auteur']">
			<meta name="DC.Creator" content="{@VALUE}"/>
		</xsl:for-each>
		<xsl:for-each select="attribute[@NAME='keywords' or @NAME='mots-clés']">
